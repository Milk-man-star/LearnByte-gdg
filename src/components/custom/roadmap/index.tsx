...
const RoadmapComponent = () => {
  ...
  const YOUTUBE_API_KEY = 'AIzaSyDR0L99BmrZs8XBJW42ulExYcMYBwern8c'; // Replace with your actual API key
  ...
  const fetchYouTubeVideos = async (query: string) => {
    ...
    try {
      ...
      setVideoError('Error fetching videos. Please try again later.');
      return [];
    } finally {
  ...
  const handleGenerateRoadmap = async () => {
    ...
    try {
      ...
      // Enhance roadmap with video recommendations
      const enhancedItems = await Promise.all(data.items.map(async (item) => {
        const enhancedTopics = await Promise.all(item.topics.map(async (topic) => {
          const enhancedSteps = await Promise.all(topic.steps.map(async (step) => {
            if (learningStyle === "visual" && !step.videoUrl) {
              const videos = await fetchYouTubeVideos(`${topic.name} ${step.detail} tutorial`);
              if (videos.length > 0) {
                return {
                  ...step,
  ...
  const renderLearningStyleButtons = () => (
    <div className="flex flex-col items-start">
      ...
      <div className="grid grid-cols-2 gap-2 w-64">
        {[
          { style: "visual", icon: Eye, label: "Visual", desc: "Videos & Diagrams" },
          { style: "reading", icon: BookOpen, label: "Reading", desc: "Articles & Docs" },
          { style: "auditory", icon: Headphones, label: "Audio", desc: "Podcasts" },
          { style: "kinesthetic", icon: HandHelping, label: "Hands-on", desc: "Practice" },
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-100">
          ...
          >
            ...
            <span className="text-sm font-medium underline underline-offset-2">
              {step.videoTitle || "Watch Tutorial Video"}
            </span>
          </a>
        </div>
      )}

      {videoLoading && <p className="text-gray-500 text-sm">Loading video suggestions...</p>}
            ...
            <div className="space-y-2">
              {step.videoUrl ? (
                ...
                >
                  ...
                  Watch Tutorial: {step.videoTitle}
                </a>
              ) : (
                <button
                  onClick={() => fetchYouTubeVideos(`${topic.name} ${step.detail} tutorial`)}
                  className="flex items-center text-blue-600 hover:text-blue-700"
                >
                  <Eye className="w-4 h-4 mr-2" />
                  Find Video Tutorial
      ...
      case "reading":
        return (
          <div className="mt-4 bg-green-50 p-4 rounded-lg">
            ...
            <div className="space-y-2">
              <a
                href={`https://en.wikipedia.org/wiki/${topic.name}`}
                target="_blank"
                rel="noopener noreferrer"
